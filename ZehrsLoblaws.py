...
class ZehrsLoblaws(BaseParseClass):
    ...
    def parse(self):
        ...
        self.driver.get(self.store_list_links[self.store_name])
        # let javascript load
        time.sleep(5)
        ...
        if self.store_name == "loblaws":
            ...
            self.driver.find_element_by_xpath(".//a[@class='btn' and @data-province='ON']").click()
            time.sleep(1)

        # get list columns of stores
        cityLists = self.driver.find_elements_by_xpath(".//div[@class='content']//div[@class='column-layout col-4']//div")
        cityURLs = []
        #storeNames = []
        #self.storeAddresses = []
        #self.storeCitys = []
        ...
        for url in cityURLs:
            ...
            viewFlyerButtons = self.driver.find_elements_by_xpath(".//a[@class='button view-flyer']")
            viewFlyerLinks = []
            # get all of the store numbers
            storeNumberElements = self.driver.find_elements_by_xpath(".//div[@class='store-listing-row']")
            ...
            addresses = self.driver.find_elements_by_xpath(".//p[@class='store-address']")
            names = self.driver.find_elements_by_xpath(".//div[@class='store-info']//h3[@class='title']")
            ...
            for i in range(len(viewFlyerButtons)):
                ...
                self.storeCitys.append(cityAndPostal[0].split(",")[0].strip())
                ...
                #print("\n\n\nname       " + str(names[i].text.encode('utf-8', 'ignore')))
                #print("store number  " + str(storeNumberElements[i].get_attribute("data-store-number").encode('utf-8', 'ignore')))
            ...
            for link in viewFlyerLinks:
                ...
                self.driver.find_element_by_xpath(".//select[@class='sortListing']//option[@value='15']").click()
                time.sleep(0.5)
                ...
                while(True):
                    try:
                        ...
                        elements = self.driver.find_elements_by_xpath(".//div[@class='card product']")
                        ...
                        for element in elements:
                            ...
                            #print(cleanPrice)
                            if weight == "":
                                print((name.text.encode('utf-8', 'ignore')) + "   price: " + cleanPrice + "    quantity: " + quantity +  "   limit: " + limit + "   each: " + each + "         add. info: " + additionalInfo)
                            else:
                                print((name.text.encode('utf-8', 'ignore')) + "   price: " + cleanPrice + "    weight: " + weight + "   limit: " + limit + "   each: " + each + "      add. info: " + additionalInfo)
                            #if not pointsAmount == "":
